...
foreach (['preferences', 'savePreferences', 'saveShiftDefaults'] as $method) {
    if (!preg_match('/#\[NoAdminRequired\]\s+public function ' . $method . '\b/s', $source)) {
        throw new RuntimeException("Angemeldeter API-Pfad {$method} fehlt.");
    }
}
if (preg_match('/#\[NoCSRFRequired\]\s+#\[NoAdminRequired\]\s+public function (savePreferences|saveShiftDefaults)\b/s', $source)) {
